...
Para isso, vamos implementar um método chamado ```getPreviousPath(Component)``` para criar um objeto de estado contento uma propriedade chamada ```prevPath```, que deverá conter o caminho da última página privada que tentou ser acessada sem um usuário logado.
